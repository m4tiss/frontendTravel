...
const MainImage = () => {
  ...
  return (
    <div className="hidden lg:flex mt-10 items-center justify-center h-[500px] w-full">
        <div className="flex flex-col items-start justify-end px-20 py-10 bg-cover h-[500px] rounded-md md:w-[1000px] xl:w-[1400px] "
        style={{backgroundImage: `url(${process.env.REACT_APP_IMAGES_URL}MainImage.jpg)`}}>  
        <h2 className="text-white text-4xl font-bold" >{t('mainImage.header')}</h2>
        <p className="text-white text-xl py-4 w-96" >{t('mainImage.description')}</p>
